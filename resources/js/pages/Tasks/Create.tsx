...
type Quadrant = "do_first" | "schedule" | "delegate" | "delete" | null;
...
const quadrantInfo = {
    do_first: {
        label: "Do First",
        desc: "Mendesak & penting — kerjakan segera!",
        badgeBg: "bg-red-500/20",
        badgeText: "text-red-400",
        borderColor: "border-red-500/40",
        cellBg: "bg-red-950/60",
    },
    schedule: {
        label: "Schedule",
        desc: "Penting tapi belum mendesak — jadwalkan dengan baik.",
        badgeBg: "bg-blue-500/20",
        badgeText: "text-blue-400",
        borderColor: "border-blue-500/40",
        cellBg: "bg-blue-950/60",
    },
    delegate: {
        label: "Delegate",
        desc: "Mendesak tapi tidak terlalu penting — pertimbangkan delegasi.",
        badgeBg: "bg-yellow-500/20",
        badgeText: "text-yellow-400",   
        borderColor: "border-yellow-500/40",
        cellBg: "bg-yellow-950/60",
    },
    delete: {
        label: "Delete",
        desc: "Tidak mendesak & tidak penting — pertimbangkan untuk dihapus.",
        badgeBg: "bg-slate-700/40",
        badgeText: "text-slate-400",
        borderColor: "border-slate-600/40",
        cellBg: "bg-slate-800/60",
    },
};
...
export default function Create({ auth, mataKuliah }: Props) {
    // ── Subtasks state ──────────────────────────────────────────
    const [subtasks, setSubtasks] = useState<{ id: number; nama: string }[]>([
        { id: 0, nama: "" },
    ]);
    ...
    const updateSubtask = (id: number, value: string) => {
        setSubtasks((prev) =>
            prev.map((s) => (s.id === id ? { ...s, nama: value } : s))
        );
    };
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        kepentingan: 3,
        kerumitan:   3,
        reminders:   ["3", "1"] as string[],
    });

    const quadrant = computeQuadrant(data.urgensi, data.kepentingan);
    const qInfo    = quadrant ? quadrantInfo[quadrant] : quadrantInfo.schedule;

    const toggleReminder = (val: string) => {
        setData("reminders",
            data.reminders.includes(val)
                ? data.reminders.filter((r) => r !== val)
                : [...data.reminders, val]
        );
    };
    ...
    const mkList = mataKuliah?.length
        ? mataKuliah.map((m) => m.nama)
        : MATA_KULIAH_DEFAULT;
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-400 mt-0.5">
                            Isi detail tugas dan parameter prioritas
                        </p>
                    </div>
                </div>
            }
            ...
            <div className="py-8 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto">
                <form onSubmit={submit}>
                    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 items-start">
                            ...
                            <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-7">
                                ...
                                <div className="mb-5">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setData("nama_tugas", e.target.value)}
                                    ...
                                    />
                                    {errors.nama_tugas && (
                                        <p className="text-xs text-red-500 mt-1">{errors.nama_tugas}</p>
                                    )}
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4 mb-5">
                                    <div>
                                        ...
                                        <select
                                            ...
                                            className={`w-full px-4 py-3 rounded-xl border text-sm font-medium bg-slate-50 text-slate-800 outline-none transition focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 cursor-pointer ${errors.mata_kuliah ? "border-red-400" : "border-slate-200"}`}
                                            required
                                        >
                                        ...
                                        {errors.mata_kuliah && (
                                            <p className="text-xs text-red-500 mt-1">{errors.mata_kuliah}</p>
                                        )}
                                    </div>
                                    <div>
                                        <label className="block text-xs font-bold text-slate-600 mb-1.5 uppercase tracking-wide">
                                        ...
                                        <input
                                            ...
                                            min={today}
                                            className={`w-full px-4 py-3 rounded-xl border text-sm font-medium bg-slate-50 text-slate-800 outline-none transition focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 cursor-pointer ${errors.deadline ? "border-red-400" : "border-slate-200"}`}
                                            required
                                        />
                                        {errors.deadline && (
                        ...
                        {/* ══════════════════════════════════════
                            RIGHT — Priority + Matrix + Reminder (2 cols)
                        ══════════════════════════════════════ */}
                        <div className="lg:col-span-2 flex flex-col gap-5">
                            ...
                            <div className="bg-slate-900 rounded-3xl p-7 text-white">
                                ...
                                <p className="text-xs text-slate-400 mb-6 leading-relaxed">
                                    Atur urgensi, kepentingan, dan kerumitan untuk posisi di Eisenhower Matrix.
                                </p>

                                {/* Urgensi */}
                                <div className="mb-5">
                                    <div className="flex justify-between items-center mb-2">
                                        <label className="text-xs font-bold text-slate-300 uppercase tracking-wide">Urgensi</label>
                                        <div className="flex items-center gap-2">
                                            <span className="text-xs text-slate-500">{sliderLabel(data.urgensi)}</span>
                                            <span className="text-base font-black text-red-400 w-4 text-right">{data.urgensi}</span>
                                        </div>
                                    </div>
                                    <input
                                        type="range" min="1" max="5"
                                        value={data.urgensi}
                                        onChange={(e) => setData("urgensi", Number(e.target.value))}
                                        className="w-full h-1.5 rounded-full appearance-none cursor-pointer accent-red-400"
                                        style={{ background: `linear-gradient(to right, #f87171 ${(data.urgensi - 1) * 25}%, #374151 ${(data.urgensi - 1) * 25}%)` }}
                                    />
                                ...
                                <div className="mb-5">
                                    <div className="flex justify-between items-center mb-2">
                                        <label className="text-xs font-bold text-slate-300 uppercase tracking-wide">Kepentingan</label>
                                        <div className="flex items-center gap-2">
                                            <span className="text-xs text-slate-500">{sliderLabel(data.kepentingan)}</span>
                                            <span className="text-base font-black text-blue-400 w-4 text-right">{data.kepentingan}</span>
                                        </div>
                                    </div>
                                    <input
                                        type="range" min="1" max="5"
                                        value={data.kepentingan}
                                        onChange={(e) => setData("kepentingan", Number(e.target.value))}
                                        className="w-full h-1.5 rounded-full appearance-none cursor-pointer accent-blue-400"
                                        style={{ background: `linear-gradient(to right, #60a5fa ${(data.kepentingan - 1) * 25}%, #374151 ${(data.kepentingan - 1) * 25}%)` }}
                                    />
                                    <div className="flex justify-between text-[10px] text-slate-600 mt-1">
                                        <span>Rendah</span><span>Tinggi</span>
                                    </div>
                                ...
                                <div>
                                    <div className="flex justify-between items-center mb-2">
                                        <label className="text-xs font-bold text-slate-300 uppercase tracking-wide">Kerumitan</label>
                                        <div className="flex items-center gap-2">
                                            <span className="text-xs text-slate-500">{sliderLabel(data.kerumitan)}</span>
                                            <span className="text-base font-black text-yellow-400 w-4 text-right">{data.kerumitan}</span>
                                        </div>
                                    </div>
                                    <input
                                        type="range" min="1" max="5"
                                        value={data.kerumitan}
                                        onChange={(e) => setData("kerumitan", Number(e.target.value))}
                                        className="w-full h-1.5 rounded-full appearance-none cursor-pointer accent-yellow-400"
                                        style={{ background: `linear-gradient(to right, #fbbf24 ${(data.kerumitan - 1) * 25}%, #374151 ${(data.kerumitan - 1) * 25}%)` }}
                                    />
                                    <div className="flex justify-between text-[10px] text-slate-600 mt-1">
                                        <span>Rendah</span><span>Tinggi</span>
                                    </div>
                                ...
                                <div className="grid grid-cols-2 gap-2.5 mb-4">
                                    {(["do_first", "schedule", "delegate", "delete"] as const).map((q) => {
                                        ...
                                        return (
                                            <div
                                                key={q}
                                                className={`rounded-2xl p-4 border transition-all duration-200 ${info.cellBg} ${info.borderColor} ${isActive ? "scale-105 shadow-lg ring-2 ring-offset-1 ring-offset-white" : "opacity-60"}`}
                                            >
                            ...
                            <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-7">
                                ...
                                <div className="flex flex-wrap gap-2 mb-4">
                                    {["3", "1", "0"].map((val) => {
                                        const active = data.reminders.includes(val);
                                        return (
                                            <button
                                                key={val}
                                                type="button"
                                ...
                                <p className="text-xs text-slate-400 flex items-center gap-1.5">
                                    ...
                                    Pastikan nomor WhatsApp sudah diverifikasi di{" "}
                            ...
                            <div className="flex gap-3">
                                ...
                                <button
                                    type="submit"
                                    disabled={processing}
                                    className="flex-[2] flex items-center justify-center gap-2 py-3.5 rounded-2xl bg-blue-600 text-white text-sm font-bold shadow-lg shadow-blue-200 hover:bg-blue-700 active:scale-95 transition disabled:opacity-60 disabled:cursor-not-allowed"
                                >
                                    {processing ? (
                                        <>
